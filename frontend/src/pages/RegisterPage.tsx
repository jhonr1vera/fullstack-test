...
export default function RegisterPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!name.trim()) {
      errors.name = VALIDATION_TEXTS.nameRequired;
    }
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                onKeyDown={(e) => handleEnterTransition(e, 'email-input')}
                placeholder={AUTH_TEXTS.register.namePlaceholder}
                disabled={isLoading}
                className={`w-full bg-slate-950/60 border ${
                  validationErrors.name ? 'border-red-500 focus:ring-red-500/20' : 'border-slate-800 focus:border-indigo-500 focus:ring-indigo-500/20'
          ...
          >
            {isLoading ? (
              <div className="flex items-center gap-2">
                ...
                <span>{AUTH_TEXTS.register.loading}</span>
              </div>
            ) : (
              AUTH_TEXTS.register.submitButton
            )}
          </button>
        </form>
      </div>
